...
> **架构方向已变更(2026-05-30)**:项目从"Drawflow 画布 + JSON 序列化"路线**转向**"DSL 脚本 + viewer runtime + 工程目录"路线。画布相关代码、UI、序列化产物**已于 2026-06-13 全部删除**(阶段 B 完成),不再投入。详见第 2、6、7 节。
>
> **MVP 范围(当前阶段唯一目标)**:
...
>
> **工程经验保护(架构再变也不要删)**:第 8 节(OPC 通讯方案,含 DI 写双向 fallback)、第 9 节(DCS 硬约束)、第 10.1 节(OPC 通信规范)、第 1 节里"Web UI 端口不要用 5000""地址归一化"这类条款,都是**踩坑得来的事务性经验**,不属于架构描述,**架构演进时不要删,只能补**。
...
6. **Web UI 端口不要用 5000**(事务性约束):viewer 默认 5002。5000 在某些 Windows/Hyper-V 环境被系统占用,会启动失败或被劫持,所以避开。(旧画布 Web 用过 5001,已随 `src/web/` 删除。)
...
#   - 默认运行周期: 1s。新工程初期允许先保留较宽 OPC 粗筛点表,项目搭好后再剔除脚本未使用点;
#                    需要压测快速闭环时,可通过 /api/script/run 显式传 dt=0.2。
...
# 工程创建向导:按容量模板创建/更新工程
#   LH3 = 1000MW, 使用 CCS_1000 协调模型
py -3.12 -m tools.create_project --name lh3 --template usc_ccs_1000mw --display LH3 --capacity-mw 1000 --model-factory CCS_1000 --force --activate
```

**点表目录约定** (viewer 自动扫描):
- 新工程标准三层目录:`projects/<name>/io/raw`(原始备份) / `projects/<name>/io/filtered`(粗筛版本) / `projects/<name>/io/simple`(最终精简版本)
- 主扫描路径:当前工程 `projects/<name>/project.yaml` 的 `io_dir` 字段,默认 `projects/<name>/io/simple`
- 回退路径:同 project.yaml 的 `io_fallback_globs` 列表;全量/粗筛点表目录用 `io_full_dir`,默认 `projects/<name>/io/filtered`
- YQ3 是历史工程,仍可显式配置 `io_dir: YQ3SIM-IO/SIMPLE/简化`;新工程不要再使用 `simple/简化` 这种双层目录
...
- 工程 = `projects/<name>/` 目录,子项:
  ...
  - `drivers/` — 可选工程级驱动规则覆盖;不存在时使用 `config/drivers/`
...
- 工程创建向导:
  - 模板目录:`projects/_templates/<template>/template.yaml`
  - CLI:`py -3.12 -m tools.create_project --name lh3 --template usc_ccs_1000mw --display LH3 --capacity-mw 1000 --model-factory CCS_1000 --force --activate`
  - API:`GET /api/project/templates`,`POST /api/project/create`
  - LH3 验收约束:`project.yaml` 中 `capacity_mw=1000`、`model_factory=CCS_1000`,脚本中使用 `CCS_1000` 且不得残留 `CCS_660`/`$YQ3`
...
3. 无扰跟踪同步:
   ...
   - CCS → `NE/PST/HM/rB` 跟踪项目实际状态:功率锚 `Ne`,主汽压反算 `pm`,分离器出口温度+压力反算 `hm`,负荷曲线反查 `rB`
   - 上载后 CCS 第一周期保持锚定输出,避免预演/运行第一周期立即被当前输入拉开
...
> **画布资产已清理(2026-06-13,阶段 B 完成)**:旧画布 Web(`src/web/`)、旧引擎(`src/sim_engine/graph_runner.py / engine.py / pairing_runner.py / ccs_model.py / demo_model.py / model.py`、旧 `py -3.12 -m src.sim_engine` CLI)、画布 config(`block_library.yaml / variables.yaml / scenarios.yaml / models/*.json / io_pairing*`)全部删除。`src/sim_engine/` 仅留 `recorder.py` + `io_pairing_gen.py`(已升格为正式工具)。详见第 7 节。
...
## 7. 当前代码现状(画布已清理,2026-06-13)

画布架构已删除(阶段 B 完成)。实际运行路线是 **DSL 脚本 + viewer runtime + 工程目录化**:
- **入口**:`py -3.12 -m src.viewer`(端口 5002),`/script` 页 DSL 赋值脚本 + OPC 实时桥接
- **模型库**:`src/models/`(`ccs_usc_otbt.py` / `steam.py` + `dsl_registry.py` 工厂注册表);红线测试 `tests/test_models_isolation.py` 锁死"不依赖 viewer/flask/asyncua/opc_client/adapter"
- **工程上下文**:`src/project.py`(工程 = `projects/<name>/` 目录,唯一真相源)
- **OPC 适配**:`src/opc_client/client.py`
- **新架构骨架**(已写,部分用作 YAML 校验):`src/engine/`(`runner.py` GraphRunner / `tagmap.py` / `recorder.py`)、`src/adapter/`、`src/models/base.py + basic.py`

**已删除**(画布资产,git 历史可查):`src/web/`(整个 Drawflow 前端)、`src/sim_engine/{graph_runner,engine,pairing_runner,ccs_model,demo_model,model}.py`、旧 `py -3.12 -m src.sim_engine` CLI、`tools/min_loop_demo.py`、画布测试、`config/{block_library,variables,scenarios}.yaml`、`config/models/*.json`、`config/io_pairing*.yaml`。

**`src/sim_engine/` 残留(已升格为正式工具,继续维护)**:
- `io_pairing_gen.py` — KKS 配对算法(`pair_analog` AQ↔AI、`pair_digital` DQ↔DI),`tools/` 和 viewer 都依赖
- `recorder.py` — 数据记录器,`src/engine/` 复用

**新功能/修改去哪里**:
| 类别 | 位置 |
|---|---|
| OPC 通信改动 | `src/opc_client/client.py` |
| 新增模型 | `src/models/` 新建类(接口 `step(...) -> tuple`)+ `dsl_registry.py` 注册成 DSL 工厂函数 |
...
| 生成器驱动规则 | 默认 `config/drivers/{vocab,devices}.yaml`;工程级覆盖 `projects/<name>/drivers/` |
| 脚本生成引擎 | `src/viewer/gen/`(`rules.py` / `generator.py` / `gateway.py`);`runtime.generate_script_from_tagmap` 仅作薄壳 |

**后续路线**(见架构讨论):驱动规则外置已完成;下一步是容量模板(`projects/_templates/`)。控制优化暂不建框架(模型库可独立 import 即留好了空间)。
...
**补充**(2026-06-13 实测):这个 write schema **会随通道状态翻转** — 同一个 `DPU3005.HW.DI010402.PV`,下装/重启后变成"只收 Boolean、拒 Float"。因此 fallback 必须**双向**(BadTypeMismatch 时换"另一边"类型重试),且 `_FORCE_FLOAT_NODES` 缓存必须**可逆**(Boolean 重试成功就把节点移出缓存),否则缓存方向跟服务器当前 schema 相反时,该节点每周期死循环写失败。单测:`tests/test_opc_write_fallback.py`。**双向 + 可逆这两点架构再变也别退化回单向**。
...
### 8.3b 新工程 OPC 点表文件级处理 ✅(绕过 GUI)

**结论**(2026-06-14 实测验证):`dcsdev.exe` 的“点表管理”GUI 不适合自动化批量导入/导出(Qt 控件不暴露稳定 UIA 控件树,DPI/窗口布局会影响坐标点击)。新工程开始阶段批量处理 OPC 通讯 IO 时,优先直接处理 NT6000 工程目录里的 `tag.csv` 点表文件。

**安装目录差异**:不同用户/机器的 NT6000 安装目录可能不同,不要把 `D:\NT6000V5` 写死为唯一位置。应先从 `dcsdev.exe` 进程路径、快捷方式目标或用户提供的安装目录确认 `NT6000_HOME`(如 `D:\NT6000V5`),下面路径均以 `%NT6000_HOME%` 表示。

**点表文件位置**(以工程 `demo`、`network3`、`DPU3001` 为例):
```text
%NT6000_HOME%\project\demo\network\network3\DPU\DPU3001\tag.csv
%NT6000_HOME%\temp\network\network3\DPU\DPU3001\tag.csv
%NT6000_HOME%\target\network\network3\DPU\DPU3001\tag.csv
```

**文件格式要点**:
- 第 1 行是 NT6000 二进制/签名头,必须原样保留,不要按普通 CSV 重写整个文件。
- 第 2 行起是 UTF-8 CSV,典型结构:
  ```text
  #VERSION,2,2025/1/30 9:27,4
  ~索引,测点名称,描述,设计编号,数据类型,...,OPC输出,...
  1,HW.AI010101.PV,燃油吹扫蒸汽压力,30HJR85CP101,FLOAT,...,Y,...
  ```
...
projects/<name>/io/filtered/  # 筛选版本:空描述/备用/非 HW 清理 + DPU 级 OPC 粗筛
...
2. 保留第 1 行二进制头,只解析/重写后续 UTF-8 CSV 数据区。
3. 将 `OPC输出` 列批量置为 `Y`。
4. 删除 `描述` 为空的点,删除 `描述` 含“备用”的点。
5. 只保留 `测点名称` 以 `HW` 开头的硬件 IO 点,删除 `SH...` 等组态/软点。
6. 重排第 1 列 `索引`,确保从 1 开始连续编号。
7. 写回后校验:行数、索引连续、`OPC输出=Y`、无空描述/备用点、无非 `HW` 点、三份文件哈希一致。

**DPU3001 实测样例**:
- 原始 989 条 → `OPC输出` 全部置 `Y`
- 删除描述为空 331 条、描述含“备用”199 条
- 删除非 `HW` 点 256 条
- 最终保留 203 条硬件 IO 点

**踩坑事务性经验**:这是一种新工程开始阶段快速整理 OPC 通讯 IO 的技术手段,不是运行时 OPC 协议逻辑。架构再变也保留:GUI 自动化只作兜底,批量点表处理优先走文件级、带备份、带校验。
...
- 动手前对照第 7 节看改动落在哪一层(模型库 / 工程上下文 / OPC 适配 / DSL runtime / viewer),是否影响可移植性。
- 提改动时,说明它在架构中的位置。
...
- 新增模型时,在 `src/models/` 写类(`step(...) -> tuple`)+ `dsl_registry.py` 注册成 DSL 工厂,并给单元测试。
- 修改 OPC 相关代码时,务必检查第 8 节硬约束(AI/DI 写 PV、写类型双向 fallback、批量读写、`raise_on_bad_status=False`)。
